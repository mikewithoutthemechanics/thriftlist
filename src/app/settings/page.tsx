  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'facebook_access_token', label: 'Facebook Access Token', placeholder: 'Enter your Facebook access token', icon: Key, type: 'password' as const },
    ...
    },
    {
      ...
      icon: Monitor,
      fields: [
        { key: 'automation_browser', label: 'Browser Type', placeholder: 'browserbase', icon: Monitor, type: 'select' as const, options: ['browserbase', 'cloakbrowser', 'playwright'] },
      ],
    },
    {
      title: 'Auto-Relist',
    ...
    },
    {
      ...
      icon: MessageCircle,
      fields: [
        { key: 'whatsapp_groups', label: 'Group Invite Links (comma-separated)', placeholder: 'https://chat.whatsapp.com/xxx, https://chat.whatsapp.com/yyy', icon: MessageCircle, type: 'text' as const },
        { key: 'whatsapp_message_template', label: 'Message Template', placeholder: '🛍️ *{title}*\n💰 R{price}\n\n📝 {description}\n\nCondition: {condition}\nSize: {size}{brand_line}{color_line}', icon: MessageCircle, type: 'text' as const, multiline: true },
      ],
    },
  ];

  const inputClass = "w-full px-4 py-3 rounded-xl border border-white/10 bg-transparent text-white placeholder-white/20 focus:outline-none focus:border-[#c4a882] text-sm font-light";
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <div className="rounded-2xl border border-white/[0.04] bg-[#111111] p-6 space-y-4">
          <div className="flex items-center gap-2">
            <Shield className="w-5 h-5 text-[#c4a882]" />
            <h3 className="font-medium text-white">Integration Status</h3>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {integrations.map(int => (
              <div key={int.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                int.status ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-white/[0.04] bg-[#0c0c0c]'
              }`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                  int.status ? 'bg-emerald-500/10' : 'bg-white/[0.04]'
                }`}>
                  <int.icon className={`w-4 h-4 ${int.status ? 'text-emerald-400' : 'text-white/20'}`} />
                </div>
                <div>
                  <p className={`text-sm font-medium ${int.status ? 'text-white/80' : 'text-white/40'}`}>{int.name}</p>
                  <p className={`text-[10px] uppercase tracking-wider ${int.status ? 'text-emerald-400' : 'text-white/20'}`}>
                    {int.status ? 'Connected' : 'Not Configured'}
                  </p>
                </div>
                {int.status && <CheckCircle className="w-4 h-4 text-emerald-400 ml-auto" />}
                {!int.status && <XCircle className="w-4 h-4 text-white/10 ml-auto" />}
              </div>
            ))}
          </div>
        </div>
        ...
        {sections.map((section, idx) => (
          <div key={idx} className="rounded-2xl border border-white/[0.04] bg-[#111111] p-6 space-y-4">
            <div className="flex items-center gap-2 mb-2">
              <section.icon className="w-5 h-5 text-[#c4a882]" />
            ...
            {loading ? <div className="space-y-3">{section.fields.map((_, i) => <div key={i} className="h-14 rounded-xl bg-white/[0.02]" />)}</div> : (
        ...
        <div className="rounded-2xl border border-white/[0.04] bg-[#111111] p-6 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            <PlayCircle className="w-5 h-5 text-[#c4a882]" />
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="px-4 py-2 bg-[#c4a882]/10 text-[#c4a882] rounded-lg text-sm hover:bg-[#c4a882]/20 transition-colors"
            >
              Replay Setup Wizard
            </button>
            <button
        ...
        <div className="flex items-center gap-2 pt-2">
          <Save className={`w-4 h-4 ${saving ? 'text-[#c4a882] animate-pulse' : 'text-[#c4a882]'}`} />
        ...
        <div className="rounded-2xl border border-[#c4a882]/20 bg-[#c4a882]/5 p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-[#c4a882] flex items-center gap-2"><Shield className="w-3.5 h-3.5" /> Security Notice</p>
