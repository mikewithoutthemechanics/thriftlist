      ...
      <div className="max-w-5xl mx-auto" ref={ref}>
        ...
        >
          ...
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground tracking-tight leading-[1.1]">
            Trusted by{" "}
            <span className="text-accent">serious resellers.</span>
          </h2>
        </motion.div>
        ...
        >
          {metrics.map((metric, i) => (
            <motion.div
              ...
              className="text-center p-6 rounded-[22px] bg-white border border-border hover:border-accent/15 hover:shadow-lg transition-all duration-300 cursor-pointer"
            >
              <metric.icon className="w-5 h-5 text-accent mx-auto mb-3" />
              <div className="text-2xl font-bold text-foreground">{metric.value}</div>
              <div className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 mt-1">{metric.label}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {testimonials.map((t, i) => (
            ...
            >
              <div className="h-full p-8 rounded-[22px] bg-white border border-border hover:border-accent/20 hover:shadow-lg transition-all duration-300">
                {/* Quote icon */}
