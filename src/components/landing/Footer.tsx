...
export default function Footer() {
  ...
  return (
    <footer className="relative py-16 px-6 bg-[#080806] border-t border-[#d4af37]/10">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-12">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="h-px w-12 bg-gradient-to-r from-transparent via-[#d4af37]/50 to-transparent"
          />
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-8 mb-10">
          {[
            ...
            <Link
              key={link.href}
              href={link.href}
              className="text-xs uppercase tracking-[0.15em] text-white/40 hover:text-[#d4af37] transition-colors duration-300 font-mono"
            >
              {link.label}
            </Link>
          ))}
        ...
        <p className="text-center text-[10px] uppercase tracking-[0.2em] text-white/25 font-light">
          <span className="text-[#d4af37]/60">©</span> {currentYear} Thrift List. Crafted in South Africa.
        </p>
      </div>
    </footer>
  );
